...
REM this one-liner enforces admin privileges (elevation), when needed; for updates & documentation see: https://github.com/SaschaHerres/batch-templates
...
REM   instance to exit, then exit itself while returning the catched exit code; this provides 100 % transparency to a cmd/script, from which this script has been called
REM details:
...
REM - this code is adapted to fit in on line to ease copying into multiple scripts; for a multiline layout some code might be different
...
REM - except for safe escaping of '"' in command line arguments, there is no input validation or escaping for non-quoted arguments (e.g.: '^', '|', '&', '<', '>', '%');
REM   test case: the following command line arguments will safely be handled and passed to the new instance: <x>.cmd "1^|" "2<2 2>2" 3 "4&o|u^r_^""
